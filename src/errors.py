...
machine-readable shape and we never leak stack traces or SQL. Kept at the package
root because both the ``api`` and ``auth`` layers raise :class:`ProblemException`.
"""
...
from store.circuit_breaker import CircuitOpenError
