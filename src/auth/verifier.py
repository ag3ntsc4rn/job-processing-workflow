"""JWT resource-server verification (used when ``AUTH_VERIFY`` is on / prod).

When enabled, the service re-verifies the Bearer JWT it receives on every
protected request even though a gateway (Apigee) already validated it upstream —
defense-in-depth so a self-minted or forged token can't be honored on a direct
call that bypasses the gateway:
...
2. verify signature + ``iss`` / ``aud`` / ``exp`` / ``nbf`` (with clock-skew
   leeway),
3. distill the claims into a :class:`~auth.principal.Principal`.

Who *signs* the token is pure config (``OIDC_ISSUER`` / ``OIDC_JWKS_URL`` /
``OIDC_AUDIENCE``): the gateway-minted JWT in prod. The network fetch is
injectable (``http_get``) so the verifier is fully unit-tested with locally
minted keys and no live IdP. In dev (``AUTH_VERIFY`` off) this module is unused;
claims are read straight from the token payload instead (see ``api/deps``).
"""
...
def _discover_jwks_url(settings: Settings, http_get: HttpGet | None) -> str:  # pragma: no cover
    if not settings.oidc_issuer:
        raise RuntimeError(
            "AUTH_VERIFY is on but neither OIDC_JWKS_URL nor OIDC_ISSUER is set"
        )
