...
Reads are **not** ownership-gated in v2: it's a machine-to-machine API, so any
caller holding ``jobs.read`` may read any job (existence is not hidden). The
``created_by`` audit trail is still recorded from the token.
"""
...
from handlerAPIv2.deps import get_store, require_read, require_write
from handlerAPIv2.errors import ProblemException
from handlerAPIv2.principal import Principal
from handlerAPIv2.schemas import CreatedBy, CreateJobRequest, CreateJobResponse, JobResponse
