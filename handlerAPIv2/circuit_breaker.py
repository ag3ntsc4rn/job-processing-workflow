...
store so handlerAPIv2's two DB calls (``enqueue`` / ``get_job``) go through the
...
from common.models import Creator, Job
...
class CircuitBreakerStore:
    """Wraps a ``Store``, routing the API's calls through a ``pybreaker`` breaker.

    Only the two operations handlerAPIv2 uses are guarded (``enqueue`` /
    ``get_job``); the other ``Store`` methods belong to other components and are
    not exercised by this API.
    """
