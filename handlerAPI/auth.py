"""OAuth2/OIDC resource-server authentication (Ping Federate).

The API validates a Bearer **access token** on every protected request:
...
Both client-credentials (machine) and authorization-code+PKCE (user) tokens are
validated identically; the principal *type* is inferred from the claims. Group
(AD) claims are captured so authorization can key off them once Ping's token
shape is confirmed.

Network fetch is injectable (``http_get``) so the verifier is fully unit-tested
with locally-minted keys and no live IdP.
"""
...
class TokenVerifier:
    ...
    def _to_principal(self, claims: dict) -> Principal:
        ...
        groups_raw = claims.get(self._settings.groups_claim) or []
        groups = tuple(groups_raw) if isinstance(groups_raw, list) else (str(groups_raw),)

        has_user_claim = any(c in claims for c in self._settings.user_claims)
        is_user = has_user_claim or (
            subject is not None and client_id is not None and subject != client_id
        )
        return Principal(
            subject=str(subject),
            principal_type="user" if is_user else "service",
            client_id=client_id,
            scopes=scopes,
            groups=groups,
        )
...
def _extract_scopes(claims: dict) -> frozenset[str]:
    ...
    # string (RFC standard) or a "scp" list (some IdPs). Support both.
